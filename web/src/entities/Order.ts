...
export default class Order extends Observable {
  ...
  addItem(item: Item) {
    const existingOrderItem = this.orderItems.find(orderItem => orderItem.idItem === item.idItem)
  ...
  removeOrderItem(orderItem: OrderItem) {
    const existingOrderItem = this.orderItems.find(o => o.idItem === orderItem.idItem)
    if(existingOrderItem) {
      existingOrderItem.quantity--
